...
function getSemesterSchedule(semesterId,url){
	if(semesterId != null && semesterId != "")
		$.ajax({
			...
			success: function(result){
				if(result.status == 200){
					var studentAppointmentDTOS = result.data.studentAppointmentList;
					//我的预约不为空  即当前是 我的预约页面
					for(var i in studentAppointmentDTOS){
						dp.events.add(setDayPilot(studentAppointmentDTOS[i]));//自主预约
					       dp.clearSelection();
					}
