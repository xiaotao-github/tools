...
dp.onEventClick = function (args){
  ...
  console.log(thisArgs)
    ...
    var scheduleId = schedule.scheduleId;//课程表Id 
    var labId = schedule.labId;//实验室Id 获取考勤
    var experimentId = schedule.experimentId;//实验Id 获取资源
    var experimentCourseId = schedule.courseId;//课程表id
  	var courseNames = args.e.data.schedule.experimentCourse.courseName;
  	var experimentNames = args.e.data.schedule.experiment.experimentName;
  	var	url ='/studentController/getCourseScheduleList/'+scheduleId+'/'+studentId+'/'+experimentId+'/'+labId+'/'+experimentCourseId;
  	var button='';
    /*postMessage(url,'排课详情');*/
	button = '<span class="toDetailPage" onclick="Hui_admin_tab(this);" data-href="'+url+'" data-title="'+experimentNames+'"></span>';
	$('.jumpBtn').html(button);
	$('.toDetailPage').click(); 
}
...
function getSemesterSchedule(semesterId,url){
	if(semesterId != null && semesterId != ""){
		$.ajax({
			...
			success: function(result){
				if(result.status == 200){
					var courseScheduleDTOS;
					if(result.data.scheduleList){
						courseScheduleDTOS = result.data.scheduleList;
					}else{
						courseScheduleDTOS = result.data.studentAppointmentList;
					}
					for(var i=0; i<courseScheduleDTOS.length; i++){
				        dp.events.add(setDayPilot(courseScheduleDTOS[i]));//整班上课
				        dp.clearSelection();
					}
				}else{
					layer.msg('请求失败！')
...
function setDayPilot(schedule){
    if(schedule.experimentName==null){
    	schedule.experimentName='';
    }

    var content ="实验："+schedule.experiment.experimentName+
        "<br/>地址："+schedule.experimentLab.labName+
        "<br/>课程："+schedule.experimentCourse.courseName+
        "<br/>教师："+schedule.teacherInfo.name;

	var type= schedule.type;
	var str='';
	var classS = '<div style="background:#f2f2f2;color:#3c817e;">';
	...
	return  new DayPilot.Event({
		start: getFormateDate(schedule.schooltimeToString),
		end: getFormateDate(schedule.schooltimeToString),
		id: DayPilot.guid(),
