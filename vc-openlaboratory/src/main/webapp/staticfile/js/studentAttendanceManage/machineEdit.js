...
function initSelectBar(){
	...
	$.ajax({
		...
		url:'/labClockInManageController/getEditMachineLabList',
		dataType: 'JSON',
		success: function(sysresult){
			if(sysresult.status == 200){
				var content = '<option value="">实验室名称</option>';
				for(var i in sysresult.data){
					if(sysresult.data[i].labId == labId){
						content += '<option value="'+sysresult.data[i].labId+'" selected="selected">'+sysresult.data[i].labName+'</option>';
					}else{
...
$("#machineEdit-form").Validform({
	btnSubmit:"#machineEditSub",
	btnReset:"#machineEditReset",
	tiptype:2, 
	ignoreHidden:false,
	dragonfly:false,
	tipSweep:false,
