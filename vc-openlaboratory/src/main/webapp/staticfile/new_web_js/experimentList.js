...
$(".eachExperiment-left span").off().on('click',function(){
	...
	if(num==0){
		$(this).children('img').attr('src',realPath+'/staticfile/images/slide-open.png');
		...
		$(this).children('img').attr('src',realPath+'/staticfile/images/slide.png');
...
$(".eachExperiment-title .span").each(function(){
	...
	var totalnum = $(this).attr("experimentTotalNum");
	if(num!=-1){
		var score = $(this).html().substring(0,num);
		/*if(totalnum!=0 || totalnum!=null){
			score = (score/totalnum)*100;
		}else{
			score=0;
		}*/
		if(score>=0&&score<60){
