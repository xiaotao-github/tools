...
$(function(){
	$.getJSON(url,function(sysresult){
		if(sysresult.status == 200){
			//资源标题
			var fileName=sysresult.data.file.fileName;
			$('.resourceName').html(fileName);
			
			//实验名称
			var exprimentName=sysresult.data.experimentGroup.experiment.experimentName;
			if(exprimentName=='' || exprimentName==null){
				暂无实验
			}
			$('.exprimentName').html(exprimentName);
			
			
			//指导老师
			var teacherInfo =  sysresult.data.experimentGroup.teacherInfo.name;
		    
		    if(teacherInfo=='' || teacherInfo==null){
		    	teacerStr='暂无指导教师';
		    }
			$('.teachers').html(teacherInfo);
			
			//是否允许下载
			var downloadStyle = '';
			if( sysresult.data.file.isDownload == 1){
				$("#isDownloadFileStyle").addClass("bc-darkGreen");//允许下载
				$("#isDownloadFileStyle").addClass("bc-hover-darkGreen");
			}else{
				$("#isDownloadFileStyle").addClass("bc-darkGray");//不允许下载
				$("#isDownloadFileStyle").addClass("bc-hover-gray");
			}
			
			
			//资源介绍
			var presentation=sysresult.data.file.filePresentation;
			...
			//资源上传时间
			var uploadTime = sysresult.data.file.createTimeToString;
			...
			var url =RESOURCE_WAY+"/"+filePath;//显示路径
			//所属学院 **
			if(sysresult.data.file.author.department != null){
				$('#college').html(sysresult.data.file.author.department.name);
			}

			var downNum = sysresult.data.file.downloadNum;
			//下载量
			$('.downloadNum').html(downNum);
		    ...
		    if(isLink==2){//资源文件
		    	if(sysresult.data.file.isExist==1){//源文件是否存在
					if(fileType=='jpg' || fileType=='png' || fileType=='jpeg' || fileType=='bmp' || fileType=='gif'){
						//图片
						content='<div style="width:778px;margin: 0 auto;height:500px;overflow:auto;"><img src="'+url+'" style="display: block;width: 100%;" /></div>';
		    			...
		    			CKobject.embedSWF(ctx+'/staticfile/lib/ckplayer/ckplayer.swf','a1','ckplayer_a1','780','500',flashvars,params);
						...
						if(resourceFileType == 'swf'){
							content='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,16,0" width="780" height="500" >'+
									' <param name="movie" value="'+url+'">'+  
									'<param name="quality" value="high">'+ 
									'<param name="play" value="true">'+ 
									'<param name="LOOP" value="false">'+ 
									'<embed style="width: 99%;margin: 0 auto;display: block;" src="'+url+'" width="780" height="500" play="true" loop="false" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash">'+ 
									'</embed>'+
								'</object>';
							$(".resourceBox").html(content);
						}else{
				    	...
				    	if(explorer.indexOf("chrome") >= 0){
							content='<div style="width:780px;height:auto;margin-left:-10px;position:relative;margin:0 auto;"><embed id="pdfPlayer" src="'+url+'" type="application/pdf" width="780px" height="500px" style=""><div style="width:762px;position:absolute;right:18px;top:0;height:55px;background:#525659;"></div></div>'; 
				    	}
				    	else if(explorer.indexOf("firefox") >= 0){
							content='<div style="width:780px;height:auto;margin-left:-10px;position:relative;margin:0 auto"><embed id="pdfPlayer" src="'+url+'" type="application/pdf" width="780px" height="500px" style=""><div style="width:150px;position:absolute;right:0;top:0;height:30px;background:#474747;"></div></div>'; 
				    	}
				    	else{
		 			    	content='<object classid="clsid:CA8A9780-280D-11CF-A24D-444553540000" width="780" height="500" border="0" style="z-index:999">'+ 
				    	...
				    	content='<div class="text-c" style="padding:100px 0px;width:100%;height:300px;background-color:#e6e6e6;">'+
							...
							'<a onclick="downLoad('+fileId+')" class="btn radius btn-success size-XL mt-40" style="margin-top:100px;" title="下载资源 ">'+
				...
				'<a  href="'+link+'" class="btn radius btn-success size-XL mt-40" style="margin-top:100px;" title="链接跳转" target="_blank">'+
