...
<div class="breadcrumb">
	<div class="inner clearfix">
		<i class="Hui-iconfont">&#xe67f;</i>系统首页<i class="c-gray en">&gt;</i>>实验笔记
...
<script type="text/javascript" > 
 ...
 $(function(){ 
	  $.ajax({
		async: false,
		type: 'post',
		url: '${ctx}/studentController/NotesList/'+'${studentId}'+'/'+'${experimentId}',
		dataType: 'json' ,  
				  ...
				  if(sysresult.data.length!=0){
					  for(var i=0;i<sysresult.data.length;i++){
						...
						var date =new Date(times);//正确
						 var notesId=sysresult.data[i].studentNotesId;
					  ...
					  $("#wish").html('<div style="width:500px;margin:0 auto;"><img style="display:block;width:100%;height:100%;" src="${ctx}/staticfile/images/null.png"></div>')
				  }
			  }
...
/* 	  for(var i=0;i<=20;i++){
	...
	$(".close").on('click',function(){
		...
		layer.confirm('确认要删除吗？',{title:'删除笔记'},function(index){
			$.ajax({
			 	type: 'POST',
			 	url: '${ctx}/studentController/delNoteById/'+noteid,
			 	dataType: 'json',
