...
<div class="page-container">
	<form action="${ctx}/experimentCourseController/updateExperimentCourse/${menuParam}" method="post" id="form-courseManage-edit" enctype="multipart/form-data" class="form">
	<input type="hidden" class="id" name="experimentCourseId" value="${experimentCourse.experimentCourseId }">
		<div class="row cl">
			<label class="col-xs-3 col-sm-3 pn-0 text-r"><span class="c-red pr-5">*</span>所属院系：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<div> 
					<select name="departmentId" datatype="* select" class="select select-box"  id="department" nullmsg="请选择所属院系！" errormsg="请选择所属院系！"">
					</select> 
				</div>
				<div class="cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text" value="${experimentCourse.courseName }" placeholder="请填写课程名称!" id="courseName" name="courseName"  nullmsg="请输入课程名称！" sucmsg="课程名称可用！"  datatype="*" errormsg="课程名称不可用！请重新输入">
				</div>
				<div class="cl">
					<span class="Validform_checktip"></span>
				</div>
	...
	<%-- 	<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8"> 
				<div>
					<input type="text" class="input-text" value="${experimentCourse.semester}" placeholder="请填写课程学期!" id="param" name="semester"  datatype="*"  sucmsg="课程学期可用！" nullmsg="请输入课程学期">
				</div>
				<div class="cl">
					<span class="Validform_checktip"></span>
				</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text" value="${experimentCourse.classHour }" placeholder="请填写总课时" id="classHour"  sucmsg="总课时可用！" name="classHour"  datatype="number" nullmsg="请输入总课时" >
				</div>
				<div class="cl">
					<span class="Validform_checktip"></span>
				</div>
		...
		<div class="row cl">
	        ...
	        <div class="formControls col-xs-8 col-sm-8">
	            <div class="col-xs-2 col-sm-2" style="width:100px;height:100px;float:left;padding:0;">
	            <c:choose>
	            	<c:when test="${empty experimentCourse.resourceFile.fileFormatPath}">
			            <img id="ImgPr" src="${RESOURCE_WAY}/system_file/img/experiment_course_default.jpg" style="width:100%;height:100%;display:block;">
	            	</c:when>
	            	<c:otherwise>
			            <img id="ImgPr" src="${RESOURCE_WAY}/${experimentCourse.resourceFile.fileFormatPath}" style="width:100%;height:100%;display:block;">
	            	</c:otherwise>
	            </c:choose>
	            </div>
        ...
        <input type="hidden" name="resourceFile.fileFormatPath" value="${experimentCourse.resourceFile.fileFormatPath}"/>
        <input type="hidden" name="resourceFile.fileId" value="${experimentCourse.resourceFile.fileId}"/>
		<div class="row cl">
			<label class="col-xs-3 col-sm-3 pn-0 text-r"><span class="c-red pr-5">*</span>课程介绍：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<textarea id="presentation" name="presentation" cols="" rows="" class="textarea"  placeholder="请输入课程介绍..." dragonfly="true"  onKeyUp="$.Huitextarealength(this,500)" ignore="ignore"></textarea>
		...
		<div class="row cl">
			<div class="col-xs-12 col-sm-12 text-c">
				<button onClick="" class="btn btn-primary radius" type="button" id="courseManage-edit-sub">
				 修改</button>
				<button onClick="" class="btn btn-secondary radius" type="button" id="courseManage-edit-reset">
...
<script>
$(function(){
	var semester = '${experimentCourse.semester}'
...
<script type="text/javascript">
	var departmentId="${experimentCourse.departmentId}"
	$(function(){
		$("#courseManage-edit-reset").click(function(){
			 var index = parent.layer.getFrameIndex(window.name);
			 parent.layer.close(index);
		})
	
		$("#form-courseManage-edit").Validform({
			btnSubmit:"#courseManage-edit-sub", 
			tiptype:2, 
			ignoreHidden:false,
			dragonfly:false,
			tipSweep:false,
	...
	//课程介绍
	var value = '${experimentCourse.presentation }';
	...
	if(menuParam==3){
		optioncontent = '<option name="" value="${experimentCourse.department.id}" selected>${experimentCourse.department.name}</option>';
		$("#department").append(optioncontent);
	}else{
		var url = '';
		if(menuParam==2){
	...
	$.getJSON(url,function(sysresult){
		if(sysresult.status == 202||sysresult.status == 400){
			$("#department option").html("无法查到您的院系信息");
		}else{
			var optioncontent = '';
			optioncontent = '<option name="name" value="" selected>请选择所属院系</option>';
				$("#department").append(optioncontent);
			for(var i=0;i<sysresult.data.length;i++){
				if(sysresult.data[i].id==departmentId){
					optioncontent = '<option name="" value="'+sysresult.data[i].id+'" selected="selected">'+sysresult.data[i].name+'</option>';
				}else{
			...
			$.ajax({
			    ...
			    success:function(sysresult){
			    	if(sysresult.status==200){
			    	 	...
			    	 	var semester ='${experimentCourse.semester}';
			    		  for(var i=0;i<sysresult.data.length;i++){
			    			  if(sysresult.data[i].semesterName != semester){
			    			  ...
			    			  var end = sysresult.data[i].endTimeToString.substring(0,sysresult.data[i].endTimeToString.indexOf(" "));
			    		       str+='<option name="semester" value="'+sysresult.data[i].semesterName+'">'+sysresult.data[i].semesterName+"("+start+"~"+end+')</option>'
			    		
			    			  }else{
			    			  var start = sysresult.data[i].startTimeToString.substring(0,sysresult.data[i].startTimeToString.indexOf(" "));
			    			  var end = sysresult.data[i].endTimeToString.substring(0,sysresult.data[i].endTimeToString.indexOf(" "));
			    		       str+='<option name="semester" value="'+sysresult.data[i].semesterName+'" selected="selected">'+sysresult.data[i].semesterName+"("+start+"~"+end+')</option>'
			    			  }
			    		  }
			    		  $('#semester').append(str);
			    		}
