...
<div class="page-container clearfix">
	<form action="${ctx}/experimentGroupController/addGroup/${cookie.vcoocUserId.value}" method="post" class="form form-horizontal" id="form-groupManage-add">
		<div class="row cl">
			<label class="col-xs-3 col-sm-3 pn-0 text-r"><span class="c-red pr-5">*</span>所在院系：</label>
			<div class="formControls col-xs-8 col-sm-8"> 
				<div>
					<input value="${department.id }" type="hidden" />
					${department.name}
				</div>
			</div>
		</div>
		...
		<div class="row cl">
			<label class="col-xs-3 col-sm-3 pn-0 text-r"><span class="c-red pr-5">*</span>所在班级：</label>
			<div class="formControls col-xs-8 col-sm-8"> 
				<div>
			...
			<label class="col-xs-3 col-sm-3 pn-0 text-r"><span class="c-red pr-5">*</span>未分配小组成员：</label>
             <div class="right-div col-xs-8 col-sm-8">
                 <div class="studentIfnoCheckbox">
              		<ul>
              			<c:choose>
              				<c:when test="${empty  studentInfoList}">
			              			<li class="f-l pr-10">
								              该班级未添加学生或学生已经在实验小组中
			              			</li>
              				</c:when>
              				<c:otherwise>
			                 	<p><label><input type="checkBox" onchange="checkAll(this);"/><span>全选</span></label></p>
		              			<c:forEach items="${studentInfoList}" var="stu">
			              			<li class="f-l pr-10">
			              				<label><input type="checkbox" name="studentInfoIds" value="${stu.id }" ><span>${stu.name }</span></label>
			              			</li>
			              		</c:forEach>
              				</c:otherwise>
              			</c:choose>
