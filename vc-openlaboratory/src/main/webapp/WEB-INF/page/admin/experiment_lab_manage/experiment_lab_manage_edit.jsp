...
<body>
	<form action="${ctx}/experimentLabController/edit" method="post" enctype="multipart/form-data"  id="form-experiment-lab-add" class="form">
	<input type="hidden" name="labManagerIds" value="">
	<input type="hidden" name="labDescription" value="">
	<input type="hidden" name="labId" value="${data.labId }">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					...
					nullmsg="请输入实验室编号！" errormsg="实验室编号不可用！请重新输入" ajaxurl="${ctx }/experimentLabController/vaildateNumber/${data.labId }">
				</div>
				<div class="cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text"  placeholder="请填写实验室名称！"  name="labName" datatype="*" value="${data.labName }"
					nullmsg="请输入实验室名称！" errormsg="实验室名称不可用！请重新输入" ajaxurl="${ctx }/experimentLabController/vaildateName/${data.labId }">
				</div>
				<div class="cl">
					<span class="Validform_checktip"></span>
				</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text"  placeholder="请填写座位数!"  name="labSeat" value="${data.labSeat }" datatype="number" nullmsg="请输入实验室座位数">
				</div>
				<div class="cl">
					<span class="Validform_checktip"></span>
				</div>
		...
		<c:if test="${menuParam == 3 }">
			<input type="hidden" name="teacherInfoIds" value="-1,${teacherInfo.id }">
		</c:if>
		...
		<div class="row cl">
	        ...
	        <div class="formControls col-xs-8 col-sm-8">
	            <div class="col-xs-2 col-sm-2" style="width:100px;height:100px;float:left;padding:0;"><img id="ImgPr" src="${RESOURCE_WAY }/${data.labImg }" style="width:100%;height:100%;display:block;"></div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text" name="mainframeId" value="${data.mainframeId }">
				</div>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text" name="mainframeKey" value="${data.mainframeKey }">
				</div>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text" name="deviceId" value="${data.deviceId }">
				</div>
			</div>
		</div>
		...
		<%-- <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-8">
				<div>
					<input type="text" class="input-text" name="videoStream" value="${data.videoStream }">
				</div>
			</div>
		</div> --%>
		...
		<div class="row cl">
			<div class="col-xs-12 col-sm-12 text-c">
				<button onClick="" class="btn btn-primary radius" type="" id="experiment_lab_edit">
...
$(function(){
	...
	//填进去介绍
	var value = '${data.labDescription }';
	value = value.replace(new RegExp("<br/>",'gm'),"\n")
	   			 .replace(new RegExp("<br/>",'gm'),"\n")
	   			 .replace(new RegExp("&nbsp;",'gm')," ");
	$("textarea[name='labDescription1']").val(value);
	//填进去状态
	$("input[name='labStatus'][value='${data.labStatus}']").attr("checked",true)
...
function getAllCheckedTeacher(){
	...
	if('${menuParam}'/1!=3){
		...
		var str = $("input[name='teacherInfoIds']").val().split(",");
		ids.push(str[1]);
	}
...
function getAllDepartment(){
	...
	if(menuParam== 3 ){
		...
		optioncontent += '<option value=${departmentId}>${departmentName }</option>';
		...
		$.getJSON(url,function(sysresult){
			 if(sysresult.status == 202||sysresult.status == 400){
				...
				for(var i=0;i<sysresult.data.length;i++){
					if(sysresult.data[i].id == "${departmentId}"/1){
						optioncontent += '<option value="'+sysresult.data[i].id+'" selected>'+sysresult.data[i].name+'</option>';						
					}else{
